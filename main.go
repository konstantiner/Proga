...
func handleFunc() {
	...
	rtr.HandleFunc("/listwriteoff", list_writeoff).Methods("GET")

	http.Handle("/", rtr)
	http.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.Dir("./static/"))))
	http.ListenAndServe(":1212", nil)
}
func list_writeoff(w http.ResponseWriter, r *http.Request) {
	type list struct {
		Id                     uint16
		Title, Anons, FullText string
		Data                   string
	...
	t, err := template.ParseFiles("templates/listwriteoff.html", "templates/header.html", "templates/footer.html")
	if err != nil {
	...
	//Выборка данных
	res, err := db.Query("SELECT * FROM `writeoffitem` ")
	if err != nil {
		panic(err)
	}

	writeoff = []list{} //очищаем список
	for res.Next() {
		var post list
		err = res.Scan(&post.Id, &post.Title, &post.Anons, &post.FullText, &post.Data)
		if err != nil {
			panic(err)
		}

		writeoff = append(writeoff, post)
	}

	t.ExecuteTemplate(w, "listwriteoff", writeoff)
